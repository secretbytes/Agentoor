...
const GetSolanaPublicKey = async( walletProvider ) => {
    ...
    const handleSubmit = async (value: string) => {
        ...
        try {
            ...
            if (imageUrl) {
                ...
                addMessage(selectedAgent.id, visionMessage)
                setChatItems(prev => [...prev, visionMessage])
            } else {
                ...
                const response = await fetch('/api/chat', {
                    ...
                    body: JSON.stringify({
                        messages: [...getMessages(currentChatId || selectedAgent.id), {
                            ...userMessage,
                            content: value + (address ? ` and my wallet address is ${address}
                                and my positionKey is ${positionKey} and publicKey is ${publicKey}` : '')
                        }],
            ...
            if (responseData.toolResponses && responseData.toolResponses.length > 0) {
                ...
                try {
                    ...
                    if (tool === "requestSwapQuote") {
                        ...
                        addMessage(selectedAgent.id, swapItem);
                        setChatItems(prev => [...prev, swapItem]);
                    }
                } catch (error) {
            ...
            const assistantMessage: ChatItem = {
                ...
                content: responseData.result || responseData.content,
                role: 'assistant',
                timestamp: Date.now()
            }
